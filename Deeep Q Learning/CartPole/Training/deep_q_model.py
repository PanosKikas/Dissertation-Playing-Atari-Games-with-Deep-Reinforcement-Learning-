...
class DQNAgent:
    def __init__(self):
        ...
        self.priority_replay_size = 30000   # max replay memory size

        self.priority_replay = Memory(self.priority_replay_size)   # create the replay memory
        self.exploration_rate = AnnealingVariable(1., .01, 20000)  # exploration rate (start_value, final_value, n_steps)
        ...
        self.update_target_frequency = 2000  # update target every
    ...
    def build_deep_q_model(self,
        ...
        cartpole_inpt = keras.layers.Input((4, ), name="cnninpt")
        # fully connected layer
        dnn = keras.layers.Dense(24, kernel_initializer=initializer, activation='relu')(cartpole_inpt)
        # mask input - one hot when want only q_value for particular action
        mask_input = keras.layers.Input((self.action_space,), name="mask")

        dnn = keras.layers.Dense(self.action_space, name="output")(dnn)

        # multiply output by mask to give true output
        output = keras.layers.Multiply()([dnn, mask_input])

        # create the model
        model = keras.Model(inputs=[cartpole_inpt, mask_input], outputs=output)
    ...
    def play(self, nepisodes, exploration_rate, stats):
        ...
        for episode in range(nepisodes):
            ...
            stats(self, episode_reward, 0, 0)
            print(episode_reward)
        stats.save_stats()
        return rewards_arr

    def save_weights(self):
        self.model.save_weights("CartPoleDQNWeights.h5")

    def restore_weights(self):
        print("Restoring model weights Pong")
        self.model.load_weights("PacmanWeightsv1.h5")


stats = Stats()
env = gym.make('CartPole-v0')
agent = DQNAgent()
agent.init_priority_replay(10000)
agent.train_model(max_steps=10e6, stats=stats)
env.close()
